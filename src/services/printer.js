export const AVAILABLE_PRINTERS = [
  { id: 'p_default', name: 'Default System Printer', type: 'system', status: 'ready' },
  ...
  { id: 'p_pos80', name: 'POS-80 Series Thermal', type: 'thermal', status: 'ready' },
  { id: 'p_pos58', name: 'POS-58 Mini Thermal (58mm)', type: 'thermal', status: 'ready' }
];
...
export function generateReceiptText(bill, settings) {
  ...
  const lines = [
    ...
    doubleLine,
    row(`Bill: ${bill.billNumber}`, bill.time || ''),
 ...
 * Renders ONLY the receipt into an isolated hidden iframe and calls print on that iframe.
 * Ensures the main browser app, UI, and navigation are never printed or disrupted.
 */
export function printReceipt(bill, settings) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const maxReceiptWidth = is58mm ? '240px' : '320px';
      ...
      // 2. Build isolated clean thermal receipt HTML
      ...
      const htmlContent = `
        ...
        <body>
          <div class="center">
            <img src="/logo.png" alt="Logo" style="height: 38px; width: auto; max-width: 160px; filter: grayscale(100%) contrast(250%); margin: 0 auto 4px auto; display: block;" />
            <div class="logo-text">${settings?.shopName || 'EAT & DRINK'}</div>
            <div class="sub-text">${settings?.shopLocation || 'MANGALAGIRI'}</div>
          </div>
      ...
      setTimeout(() => {
        try {
          ...
          // Fallback to window print if iframe print blocked
          window.print();
          resolve({ success: true, billNumber: bill.billNumber });
        }
      }, 150);
